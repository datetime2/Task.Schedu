...
namespace Task.Schedu.Utility
{
    internal static class DbHelper
    {
        internal static int ExecuteNonQuery(string strCon, string strSQL, object param)
        {
            using (MySqlConnection con = new MySqlConnection(strCon))
            {
                if (con.State != ConnectionState.Open)
                    con.Open();
                MySqlCommand cmd = GetSqlCommand(strSQL, param);
                cmd.Connection = con;
        ...
        internal static DataTable FillDataTable(string strCon, string strSQL, object param)
        {
            ...
            {
                var cmd = GetSqlCommand(strSQL, param);
                cmd.Connection = con;
                if (con.State != ConnectionState.Open)
                    con.Open();
        ...
        internal static DataTable RunProcedure(string strCon, string procName, object param)
        {
            using (var con = new MySqlConnection(strCon))
            {
                var cmd = GetSqlCommand(procName, param);
                cmd.Connection = con;
                if (con.State != ConnectionState.Open)
                    con.Open();
        ...
        /// <returns>T</returns>
        private static T GetSingleT<T>(MySqlDataReader dr)
        {
            Type type = typeof(T);
            ...
            for (int i = 0; i < dr.FieldCount; i++)
            {
                ...
                if (pInfo != null)
                {
                    ...
                    if (!ptype.IsEnum)
                        //if (ptype.Name.ToUpper().Equals("GUID"))
                        //{
                        //    pInfo.FastSetValue(obj, Guid.Parse(dr[i] + ""));
                        //}
                        //else
                        //{
        ...
        public static MySqlCommand GetSqlCommand(string strSQL, object argsObject)
        {
