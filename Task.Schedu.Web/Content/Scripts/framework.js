    ...
    return {
        ...
        parseDate: function (s, ignoreTimezone) {
            ...
            if (typeof s == 'string') {
                ...
                var d = Schedu.parseISO8601(s, ignoreTimezone) || (s ? new Date(s) : null);
                return isNaN(d) ? null : d;
            }
        ...
        */
        formatDate: function (date, format) {
            ...
            var dateFormat = Schedu.dateInfo;
            if (!dateFormat) dateFormat = Schedu.dateInfo;
                ...
                switch (ctl.nodeName) {
                    case "INPUT":
                        ...
                        if ($(ctl).hasClass("layer-date")) {
                            options = Schedu.getDataOptions(ctl);
                            if (options.isInit) {
                                $(ctl).dateTime(options);
                            }
                        }
                        else if ($(ctl).hasClass("Schedu-PlateNo")) {
                            //初始化车牌选择
                            //获取初始化选项
                            options = Schedu.getDataOptions(ctl);
            ...
            this.setData = function (data) {
                ...
                if (mode == WebState.READ) {
                    $(formCtl.find('.readSpan')).each(function (i, item) {
                        ...
                        options = Schedu.getDataOptions(item);
                        value = data[name];
                        if (options.format) {
                            if (options.format.indexOf("YYYY") == 0) {
                                value = data[name].formatDateString(Schedu.parseDateFormat(options.format));
                            }
                        }
                        $(item).text(value);
                    });
                            ...
                            switch (input.type) {
                                ...
                                case "text":
                                    if ($(input).hasClass("layer-date") || $(input).hasClass("formatDate")) {
                                        options = Schedu.getDataOptions(input);
                                        if (options.format) {
                                            input.value = data[name].formatDateString(Schedu.parseDateFormat(options.format));
                                        } else {
...
(function ($) {
    var arrId = [];
    var arrType = { "datetime": "Schedu_dateTime" };
    $.fn.extend({
        ScheduId: function (type) {
            var id = $(this).attr("id");
            if (!id) {
        ...
        */
        dateTime: function (options, callback) {
            ...
            $(this).ScheduId(arrType.datetime);
            var id = $(this).attr("id");
...
jQuery.extend({
    ...
    ExportXLS: function (options) {
        if (options.columnInfos && options.columnInfos.length > 0) {
            ...
            if (!$.isArray(options.data)) {
                //查询条件
                var queryParam = Schedu.clone(options.condition);
                //最多导出50000条数据
    ...
    * 参数：    type：业务类型 取值参照 Task.Schedu.Utility.Excel.ExcelImportType 枚举
    * 返回值：  无
    * 创建人：  焰尾迭
    * 创建时间：2016-08-19
    */
    ...
    * 参数：    options：
                {
                    type:业务类型, 取值参照 Task.Schedu.Utility.Excel.ExcelImportType 枚举
...
$.extend(String.prototype, {
    ...
    */
    ...
    toDate: function () {
        if (this) {
            return Schedu.parseDate(this.toString());
        }
        return null;
    },
    formatDateString: function (format) {
        ...
        if (this) {
            return Schedu.formatDate(Schedu.parseDate(this.toString()), format);
        }
        return null;
    },
